// score-cat-wet.js — Calculate transparency scores for all wet cat food products
// Scores 0-100 based on ingredient quality, label transparency, and nutritional data
// Uses wet-specific protein thresholds and filler lists
// Run: node server/score-cat-wet.js
...
const FILLER_INGREDIENTS = [
  ...
  "powdered cellulose",
  // Wet-specific fillers
  "water sufficient for processing", "modified corn starch",
...
function scoreProduct(product) {
  ...
  if (ingredients.length > 50) score += 2;
  if (ga.length > 30) score += 2;
  if (aafco.length > 10) score += 2;
  if (product.calorieContent) score += 2;
...
// ─── Run scoring ─────────────────────────────────────────────────────────────
const products = db.prepare("SELECT * FROM products_petsmart_cat_wet").all();
const update = db.prepare("UPDATE products_petsmart_cat_wet SET transparencyScore = ? WHERE id = ?");
...
// Show distribution
const dist = db.prepare("SELECT MIN(transparencyScore) as min, MAX(transparencyScore) as max, AVG(transparencyScore) as avg FROM products_petsmart_cat_wet").get();
console.log(`Scored ${scored} wet food products.`);
console.log(`Distribution: min=${dist.min}, max=${dist.max}, avg=${Math.round(dist.avg)}`);

// Show some examples
const examples = db.prepare("SELECT name, brand, transparencyScore FROM products_petsmart_cat_wet ORDER BY transparencyScore DESC LIMIT 5").all();
console.log("\nTop 5:");
examples.forEach(p => console.log(`  ${p.transparencyScore} — ${p.name} (${p.brand})`));

const bottom = db.prepare("SELECT name, brand, transparencyScore FROM products_petsmart_cat_wet ORDER BY transparencyScore ASC LIMIT 5").all();
console.log("\nBottom 5:");
